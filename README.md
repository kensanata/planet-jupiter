...
entry newer than 90 days." This is set when updating the feeds in your cache.
...
Jupiter that matches the status message (such as 206 for a "partial
content" status when there aren't any recent entries in the feed). This is set
when updating the feeds in your cache.
- `title` is the site's title. When you update the feeds in your cache, the title
...
info in the HTTP response that isn't available from the feed itself.
...
**date** is the publication date of the HTML page, in ISO date format:
YYYY-MM-DD.
